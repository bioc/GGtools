...
chr20GGceu
@
The \Sexpr{class(chr20GGceu)} class is an extension of the
...
The data are organized into an \Sexpr{nrow(exprs(chr20GGceu))}
by \Sexpr{ncol(exprs(chr20GGceu))} matrix of expression values
accessible with the \texttt{exprs} method, and
an \Sexpr{nrow(snps(chr20GGceu))} by \Sexpr{ncol(snps(chr20GGceu))}
...
<<do20>>=
S100 = snpScreen(chr20GGceu, chr20meta, genesym("CPNE1"), ~., lm, gran=25) 
S100
@
...
ut
sIRF5
wm = which.min(pp <- extract_p(sIRF5))
pp[wm]
<<dofig5,fig=TRUE>>=
...
<<lkevg,fig=TRUE>>=
 par(mfrow=c(1,2))
 plot_EvG(chr7GGceu, genesym("IRF5"), "rs2280714")
 plot_EvG(chr7GGceu, genesym("IRF5"), "rs6969930")
@
