...
allele count in the data to determine the code unambiguously.]
...
<<getd>>=
nn = getNamedLocs(slpack= "SNPlocs.Hsapiens.dbSNP.20100427", chr="ch20")
library(GenomicRanges)
gg = getGRanges( f1, 1, 1, "ch20", nn )
gg[1:3,]
@

We convert this to a RangedData instance for viewing on the UCSC genome browser.
This code will work in a clean-enough R session; in the sequence
of computations given in this vignette it
can throw an infinite recursion.
<<lkb,eval=FALSE>>=
if (interactive()) {
s1 = browserSession()
rd = RangedData(gg)
genome(rd) = "hg18"
s1[["CPNE1"]] = rd
v1 = browserView(s1, GenomicRanges(33.5e6,34.5e6, "chr20"), track=ucscTrackModes(hide="knownGene", full=c("refGene", "CPNE1")))
}
@

\section{Using imputation to 1000 genomes loci}

The \textit{snpMatrix} package defines methods for using
...
certain set of calls for CEU, perhaps HapMap phase III.  The \textit{ceu1kg}
experimental data package of Bioconductor includes the results
of this application of MACH
for chromosome 20 of CEPH CEU.
<<lkmach>>=
library(ceu1kg)
if (!exists("C20MLDOSE")) data(C20MLDOSE)
dim(C20MLDOSE)
C20MLDOSE[1:5,10:15]
@
Note that at this time we do not have any mechanism for high-performance
testing using the dose representation in C20MLDOSE.

The observed genotypes for CEU founders from HapMap phase II are
<<getcc>>=
c20obs = smList(hmFouc)[[1]]
c20obs
@
We impute as follows:
<<geti>>=
unix.time(c20imp <- impute.snps( imphm2_1KG_20_mA2, c20obs ))
c20imp[1:5,1:5]
@
The results need to be placed in compatible structures.
We will use C20D to hold the allele dose measurements from 
MACH and R20D to hold the snpMatrix regression-based expected allele counts.
<<lkcom,keep.source=TRUE>>=
tc20imp = t(c20imp)  # cols are samples
                     # restrict to common samples
C20D = C20MLDOSE[, intersect(colnames(C20MLDOSE), colnames(tc20imp))]
R20D = tc20imp[, intersect(colnames(C20MLDOSE), colnames(tc20imp))]
...
<<lkim>>=
length(mimp <- setdiff(rownames(C20MLDOSE), colnames(c20obs)))
@
...
sum(vn < 0.01)
@
The latter result shows that there are over 7000 SNP for which MACH
is willing to impute a single het against 109 homozygous individuals.
Let's look at an example:
<<ex>>=
table(news[ vn > 0 & vn < 0.01, ][1,])
@
...
length(inmachNR)
C20MLDOSE[inmachNR[1:5],1:10]
table(C20MLDOSE[inmachNR[1],])
table(C20MLDOSE[inmachNR[2],])
library(ceu1kg)
...
@
So MACH is able to call 2 hets for each of these loci.
We verify that snpMatrix regression does not
<<lkm>>=
