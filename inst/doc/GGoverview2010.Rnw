...
(in package \Rpackage{snpMatrix2}) allowing reasonably
...
of the \Rclass{SnpMatrix} class from \Rpackage{snpMatrix2}.
...
The \textit{snpMatrix2} package defines methods for using
...
MACH and R20D to hold the snpMatrix2 regression-based expected allele counts.
...
Let's now consider loci imputed by MACH but not by snpMatrix2.
<<lkn>>=
...
We verify that snpMatrix2 regression does not
<<lkm>>=
if (.Platform$OS.type != "windows" ) {
