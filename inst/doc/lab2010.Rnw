...
@
You are now managing and interrogating 17 million test results.  By increasing
...
\textbf{Question 6.}  You can use \texttt{as.ram} to work directly with
...
s[1] = which.max(apply(as.ram(e1@fflist[[1]]),1,max))
g[1] = which.max(apply(as.ram(e1@fflist[[1]]),2,max))
s[2] = which.max(apply(as.ram(e1@fflist[[2]]),1,max))
g[2] = which.max(apply(as.ram(e1@fflist[[2]]),2,max))
s[3] = which.max(apply(as.ram(e2@fflist[[1]]),1,max))
g[3] = which.max(apply(as.ram(e2@fflist[[1]]),2,max))
s[4] = which.max(apply(as.ram(e2@fflist[[2]]),1,max))
g[4] = which.max(apply(as.ram(e2@fflist[[2]]),2,max))
ind = which.max( c(
   e1@fflist[[1]][s[1],g[1]],
   e1@fflist[[2]][s[2],g[2]],
   e2@fflist[[1]][s[3],g[3]],
