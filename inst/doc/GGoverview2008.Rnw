...
all chromosomes.  These computations cannot be done on the
standard windows machine.  Please see a unix-generated vignette
for results.
<<getddd>>=
...
<<dodwg,fig=TRUE>>=
if (ONWIN) plot(1) else plot(rr)
@

\section{Phenotype data available on CEPH samples}
<<lkpd>>=
if (!ONWIN) names(pData(hmceuB36))
@
