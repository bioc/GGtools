...
names(grl) = names(ps)
@
SNP locations can be obtained as follows.
<<getsr>>=
...
seqnames(c20) <- sub("ch", "", seqnames(c20))
names(c20) = paste("rs", elementMetadata(c20)$RefSNP_id, sep="")
c21 = getSNPlocs("ch21", as.GRanges=TRUE)
seqnames(c21) <- sub("ch", "", seqnames(c21))
names(c21) = paste("rs", elementMetadata(c21)$RefSNP_id, sep="")
