...
system("rm -rf foobarr")
system("mkdir foobarr")
e1 = eqtlTests(hmlit, ~male, targdir="foobarr", runname="foobarr")
e1
class(e1)
@
...
seqnames(c20) <- sub("chr", "", seqnames(c20))
...
names(c21) = rs21
seqnames(c21) <- sub("chr", "", seqnames(c21))
