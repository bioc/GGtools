...
<<lkgsc>>=
top200 = g20ok[order(elementMetadata(g20ok)$score, decreasing=TRUE)[1:200]]
top200[196:200]
@
...
<<doext>>=
if (interactive() & file.exists("exdempack")) stop("will not remove existing 'exdempack' folder")
system("rm -rf exdempack")
externalize(hmceuB36.2021, "exdempack")
tar("exdempack.tar.gz", "exdempack", compression="gzip")
install.packages("exdempack.tar.gz", repos=NULL, type="source")
@
Now we can use \texttt{getSS} to create tailored smlSet instances on the fly.
<<lkd>>=
gc()
s1 = getSS("exdempack", "20")
gc()
rm(s1)
gc()
@
...
names(mymap) = c("20", "21")
unix.time(md <-  makeDiagDirector("exdempack", mymap, rhs=~male, sleeplen=2, runname=tname()))
md
@
