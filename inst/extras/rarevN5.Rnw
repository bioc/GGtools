...
rc = rc[which(rcs>0)]
@
We also drop genes with relatively low variation.
<<filt>>=
ex = exprs(ceu6)[names(rc),]
mads = apply(ex,1,mad)
ok = which(mads>median(mads))
rc = rc[ ok ]
@
...
library(MASS)
for (i in 1:9) {
 plot( exprs(ceu6)[gd[i], ] ~ jitter(rc[[gd[i]]]), main=gd[i],
   ylab = get(gd[i], illuminaHumanv1SYMBOL), xlab="# rare SNP minor alleles in gene" ) 
 rlmout = lqs(exprs(ceu6)[gd[i], ] ~ rc[[gd[i]]])$coef
 co = lms[[ gd[i] ]]
 rco = rlmout
 abline(co[1], co[2], lty=2, col="gray", lwd=2)
 abline(rco[1], rco[2], lty=4, col="gray", lwd=2)
}
@
