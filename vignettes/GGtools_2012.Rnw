...
We use ``filter'' to refer to any function that processes an \texttt{smlSet}
...
\texttt{eqtlTests} function.  To illustrate, we sharply filter
the expression data to the 50 most variable genes on chromosome 20 as
measured by cross-sample median absolute deviation, filter away SNP
with MAF $<$ 0.05, and
then execute all tests.

<<do20,cache=TRUE,keep.source=TRUE>>=
g20 = restrictProbesToChrom(g20, "20")
mads = apply(exprs(g20),1,mad)
oo = order(mads, decreasing=TRUE)
g20 = g20[oo[1:50],]
library(parallel)
e1 = eqtlTests(MAFfilter(g20, lower=0.05), ~male, 
    geneApply=mclapply, targdir="dem1")
e1
@
...
obtained in the \Sexpr{length(probesManaged(e1,1))*length(snpsManaged(e1,1))}
tests computed, in an \texttt{ff} archive.  We use \texttt{topFeats}
to manually harvest this.

<<gettop,cache=TRUE>>=
pm1 = probesManaged(e1,1)
tops = sapply(pm1, function(x) topFeats(probeId(x), mgr=e1, ffind=1, n=1)) 
top6 = sort(tops, decreasing=TRUE)[1:6]
print(top6)
@

R has propagated the names of probes and SNPs with the scores so that we
...
Nevertheless, we can check consistency of these findings with other published
results involving multiple populations.
...
@
Thus the top two SNP in our table are identified as multipopulation eQTL by
Stranger.  The other association scores are not very strong and likely do
not correspond to genuine associations.

\subsection{Tabulating best associated rSNP with permutation-based FDR}

b1 = best.cis.eQTLs("GGdata", ~male,  genome="hg19", useTxDb=TRUE,
   folderstem="db2", nperm=2, geneApply=mclapply,
   exTransform= function(x)
    MAFfilter(x[probeId(featureNames(g20)),], lower=0.05),
   dropChr=c(as.character(1:19),"21", "22", "X", "Y"))
