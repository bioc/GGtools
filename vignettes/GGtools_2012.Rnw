...
g20f = exTx(g20)
b2 = best.cis.eQTLs("GGdata", ~male,  genome="hg19", 
   useTxDb=TRUE, cisRadius=1e6,
   folderstem="db2", nperm=2, geneApply=mclapply,
   exTransform= exTx,
   dropChr=c(as.character(1:19),"21", "22", "X", "Y"))
...
function
<<ggg>>=
goodProbes = function(x) names(x@obsrd[elementMetadata(x@obsrd)$score>3])
@
...
np = length(newp)
bestSnp = function(pn, esm) elementMetadata(esm@obsrd[pn])$rsid
par(mfrow=c(2,2))
