...
pdf(file="t1.pdf")
plot(t1, "SNPlocs.Hsapiens.dbSNP.20100427")
dev.off()
pdf(file="t1evg.pdf")
...
but here we create an alternative table for the sharply filtered chromosome 20 data
...
b1 = best.cis.eQTLs("GGdata", ~male,  genome="hg19", 
   useTxDb=TRUE, cisRadius=1e6,
...
of principal components of variation.  We maximize information
on batch effects by working from the entire expression matrix,
subsetting to desired samples and probes subsequently.
