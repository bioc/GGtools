...
%\VignetteDepends{GGdata, SNPlocs.Hsapiens.dbSNP.20120608}
...
<<doc,cache=FALSE,keep.source=TRUE>>=
...
<<doall1>>=
b1.all = GGtools:::All.cis.eQTLs( maxfdr = 0.05, inbestcis = b1, smpack="GGdata", rhs=~1, 
 chrnames = "20",
...
Nota bene.  Many users have attempted to work with All.cis.eQTLs with maxfdr set to 1
in order to obtain all cis tests.  This is not the intended use.  eQTLtests or a new function called
All.cis can be used for exhaustive testing tasks.
...
<<domo,cache=FALSE, keep.source=TRUE>>=
if (file.exists("db2")) unlink("db2", recursive=TRUE)
g20 = getSS("GGdata", "20")
...
For windows system we use a smaller radius and fewer genes
to live within a limited memory footprint.
<<lkc2,cache=FALSE>>=
therad = 1e6
if (.Platform$OS.type == "windows") {
   therad = 5e4
   exTx = function(x) MAFfilter( clipPCs(x,1:10)[probeId(fn[1:20]), ], lower=0.05)
   g20f = exTx(g20)
 }
@
At the moment this does not succeed in windows systems.
<<lkb22>>=
if (.Platform$OS.type != "windows") {
b2 = best.cis.eQTLs("GGdata", ~male,  radius=therad,
...
if (.Platform$OS.type == "windows") {
plot(0,0,main="these computations unavailable on windows system")
}
@
