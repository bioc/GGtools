...
cisTo = function (sr, gr, rad = 2e+06) 
{
    ...
    if (length(unique(seqnames(sr))) > 1) stop("need sr to represent only one chromosome")
...
getTransScores = function(snpname, mgr, cisToList=NULL, ffind=1, n=200, searchfac=4 ) {
#
...
#
 ...
 sort(t1, decreasing=TRUE)[1:min(c(length(t1),n))]
}
